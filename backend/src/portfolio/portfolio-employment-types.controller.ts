...
@Controller('portfolio/employment-types')
export class PortfolioEmploymentTypesController {
  constructor(private readonly svc: PortfolioEmploymentTypesService) {}
  ...
  @Post()
  create(
    @Body() body: { name: string; is_active?: boolean; display_order?: number },
    @Req() req: any,
  ) {
    const tenantId = req?.tenant?.id ?? '';
