...
export const DEFAULT_DASHBOARD_CONFIG: DashboardTileConfig[] = [
  ...
  { id: 'recently-viewed', enabled: true, order: 4, config: { limit: 10 } },
  { id: 'my-time-last-week', enabled: true, order: 5, config: { days: 7 } },
  { id: 'new-requests', enabled: true, order: 6, config: { limit: 5, days: 7 } },
  // Phase 2 tiles - disabled by default
  { id: 'team-activity', enabled: false, order: 7, config: { limit: 10 } },
  { id: 'global-status-changes', enabled: false, order: 8, config: { days: 5 } },
  { id: 'stale-tasks', enabled: false, order: 9, config: { scope: 'my', thresholdDays: 90 } },
];
