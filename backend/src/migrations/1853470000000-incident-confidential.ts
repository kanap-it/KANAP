...
/**
 * Incident register: confidential flag + omit confidential titles from the
 * tasks search index (related-object label is INC-N only).
 *
 * Search-function body follows 1853460000000 (with-incident) and only changes
 * the weight-C related-incident title.
 */

const TASKS_REFRESH_WITHOUT_CONFIDENTIAL = `
...
const TASKS_REFRESH_WITH_CONFIDENTIAL = `
  CREATE OR REPLACE FUNCTION search_index_refresh_tasks(p_tenant uuid, p_ids uuid[] DEFAULT NULL)
  RETURNS void AS $fn$
    DELETE FROM search_index si_del
    WHERE si_del.tenant_id = p_tenant
...
export class IncidentConfidential1853470000000 implements MigrationInterface {
  name = 'IncidentConfidential1853470000000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    `);
    await queryRunner.query(TASKS_REFRESH_WITH_CONFIDENTIAL);
    await queryRunner.query(TASKS_SYNC_TRIGGER);
    await queryRunner.query(REFRESH_ALL_TASKS_TENANTS);
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(TASKS_REFRESH_WITHOUT_CONFIDENTIAL);
    await queryRunner.query(TASKS_SYNC_TRIGGER);
    await queryRunner.query(REFRESH_ALL_TASKS_TENANTS);
    await queryRunner.query(`ALTER TABLE incidents DROP COLUMN IF EXISTS confidential`);
  }
}
