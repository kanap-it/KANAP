...
export function classificationSqlExpressions(alias = 'a'): Record<string, string> {
  ...
  const complete = `NULLIF(BTRIM(${a}.criticality), '') IS NOT NULL AND ${a}.cyber_criticality IS NOT NULL AND ${a}.data_class IS NOT NULL AND ${a}.recovery_wave IS NOT NULL AND NULLIF(BTRIM(${a}.classification_justification), '') IS NOT NULL`;
  return {
    ...
    classification_review_state: `(CASE WHEN NOT (${complete}) THEN 'incomplete' WHEN ${a}.classification_review IS NOT NULL AND (${a}.classification_review->>'revision')::integer = ${a}.classification_revision THEN 'reviewed' ELSE 'stale' END)`,
