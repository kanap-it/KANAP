 ...
 * 7-day dedupe must only record a recipient who is actually emailed, so a user who opts in
 * after a skipped run still gets the next warning.
 */
...
async function warn(svc: NotificationsService, itemType: 'contract' | 'opex', recipients: typeof ALICE[]) {
