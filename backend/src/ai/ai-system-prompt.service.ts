...
function toolUsageGuidelines(mode: AiContextProfile['promptMode']): string {
  const core = [
    ...
    '- Application classifications: read get_application_classification_catalog for complete tenant codes, labels, definitions, severity ranks and recovery order. get_filter_values returns observed values, not the catalog. Use maximum catalog rank (including deprecated historical levels for reads, active levels only for new assignments); return zero when no accessible application has that level, without falling back to the observed maximum. Use exact query filters, combined criteria and aggregate_entities counts, and paginate before claiming a complete list. Link applications to their /compliance tab.',
    '- Business criticality is derived only from business_mtd_minutes (MTD/DMIA). New or changed MTD values must be one of the tenant businessMtdPresets returned by get_application_classification_catalog; null clears it. Durations are whole minutes; one day is 1440 and one week is 10080. Cyber is a manual consequence assessment; confidentiality and recovery wave are independent. An unspecified classification stays null. If criticality is ambiguous between business and cyber, reuse an explicit conversational axis or ask which axis. Never infer MTD from a legacy level or fabricate justification or review.',
    '- Application create/update previews accept business_mtd_minutes, cyber_criticality, data_class, recovery_wave, rto_minutes, rpo_minutes and classification_justification, including explicit null to clear one field. Use tenant codes. A preview displays derived business criticality and review invalidation; changed method versions or revisions require a new preview. Catalog changes and human review are not AI write capabilities.',
