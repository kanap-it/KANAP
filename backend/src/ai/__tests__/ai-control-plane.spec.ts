...
async function testAgentPersonaRejectsOverLimitAndClearsEmptyFields() {
  ...
  await assert.rejects(
    () => service.updateAgentDefinition(context, definition.id, {
      persona_json: {
        instructions: Array.from({ length: 17 }, (_entry, index) => `Instruction ${index + 1}`),
      },
    }),
    (error: unknown) => error instanceof BadRequestException
      && (error as BadRequestException).message === 'Instructions can have at most 16 lines.',
  );
  await assert.rejects(
    () => service.updateAgentDefinition(context, definition.id, {
      persona_json: {
    ...
    (error: unknown) => error instanceof BadRequestException
      && (error as BadRequestException).message === 'Each instruction can be at most 500 characters.',
  );
  await assert.rejects(
    () => service.updateAgentDefinition(context, definition.id, {
      persona_json: {
