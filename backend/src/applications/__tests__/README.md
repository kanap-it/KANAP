...
The main integration suite rolls back its fixtures. The concurrency suite commits a uniquely named temporary tenant for visibility across connections and removes it afterwards. It checks publication/write serialization, that saving the catalog never moves an application or invalidates its review, and CSV failure rollback. The migration fixture verifies that historical criticality values and tenant confidentiality ordering survive without invented reviews.
...
Frontend duration, configured-choice, review and settings editor tests are in `frontend/src/pages/it/components/`. Browser evidence from isolated French test data is in `doc/assets/application-classification-v1-light.png` and `doc/assets/application-classification-v1-dark.png`. The recorded browser smoke covers choosing a business level, explicit review, named reviewer and navigation to the single Knowledge block. External live Plaid model generation is not covered; tool discovery, filters, preview/mutation contracts and guards are exercised deterministically.
