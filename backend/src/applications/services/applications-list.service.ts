...
export class ApplicationsListService extends ApplicationsBaseService {
  ...
  async list(query: any, opts?: ServiceOpts) {
    ...
    // Batched: one set of queries for the whole page instead of 2-5 per application.
    const derivedUsersByApp = await this.buildDerivedUsersByApp(entities, mg);
    ...
    const items = await Promise.all(
      entities.map(async (app, idx) => {
        ...
        base.derived_total_users = derivedUsersByApp[app.id] ?? 0;
        return base;
      }),
    );
  ...
  /**
   * Batched counterpart of the former per-row derived-users computation.
   *
   ...
   */
  private async buildDerivedUsersByApp(apps: Application[], mg: any): Promise<Record<string, number>> {
    ...
    for (const app of apps) {
      ...
      let total = 0;
      for (const companyId of appCompanyIds) {
