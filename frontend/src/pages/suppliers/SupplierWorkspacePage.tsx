...
export default function SupplierWorkspacePage() {
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 2 }}>
        <Stack>
          <Typography variant="h6">
            {isCreate ? 'New Supplier' : (data?.name || 'Supplier')}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {!isCreate && total > 0 ? `Supplier ${index + 1} of ${total}` : ''}
          </Typography>
        </Stack>
      ...
      {!!error && <Alert severity="error">Failed to load supplier.</Alert>}
        ...
        <Box sx={{ flex: 1, pl: 3 }}>
          {isCreate && (
            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 1 }}>
              The Contacts tab becomes available after you create the supplier.
            </Typography>
          )}
