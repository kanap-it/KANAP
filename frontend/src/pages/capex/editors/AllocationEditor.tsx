...
export default forwardRef<AllocationEditorHandle, Props>(function AllocationEditor({ id, year, availableYears, onYearChange, onDirtyChange }, ref) {
  ...
  const computeManualCompanyAllocations = React.useCallback(
    async (inputRows: AllocationRow[], driver: 'headcount' | 'it_users' | 'turnover') => {
      ...
      if (sum <= 0) { throw new Error(t('capex.allocations.provideDriverForCompanies', { driver: driver.replace('_', ' ') })); }
      let accPct = 0;
  ...
  const recalcDeptAlloc = React.useCallback(async (baseline: AllocationRow[]) => {
    try {
      ...
      let missing = false;
      const weights = selected.map(r => { const id = r.department_id as string; const w = Number(hcByDept[id] || 0); if (!Number.isFinite(w) || w <= 0) missing = true; return { id, w }; });
      if (missing) { setError(t('capex.allocations.provideHeadcountForDept')); setRows(baseline); return; }
      const sum = weights.reduce((acc, it) => acc + it.w, 0);
      if (sum <= 0) { setError(t('capex.allocations.headcountSumZero')); setRows(baseline); return; }
  ...
  const ensureVersion = React.useCallback(async (): Promise<Version> => {
    ...
    const body = { version_name, budget_year: year, as_of_date: `${year}-01-01`, input_grain: 'annual', notes: null };
    const created = await api.post<Version>(`/capex-items/${id}/versions`, body);
  ...
  const recalcForRemaining = async (remaining: AllocationRow[], driver: 'headcount' | 'it_users' | 'turnover') => {
    try {
      ...
      setError(err?.message || t('capex.allocations.failedToRecompute'));
      setRows(remaining);
    }
  };
    ...
    async (versionId: string, expectedRows: ManualPayloadRow[]) => {
      ...
      try {
        const [versionsRes, allocRes] = await Promise.all([
          api.get<Version[]>(`/capex-items/${id}/versions`),
          api.get<{ items: any[]; resolved_method?: string }>(`/capex-versions/${versionId}/allocations`),
        ]);
        ...
        if (mismatch || (backendMethod && backendMethod !== method) || (resolvedMethod && resolvedMethod !== method)) {
          console.error('[CapexAllocationEditor] Save verification mismatch', debugPayload);
          return { ok: false, ...debugPayload };
        }
        console.debug('[CapexAllocationEditor] Save verification ok', debugPayload);
        return { ok: true, ...debugPayload };
      } catch (err) {
        console.error('[CapexAllocationEditor] Save verification failed', err);
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      setVersion(v ?? null);
      if (v) {
        const alloc = await api.get<{ items: any[]; total_pct: number }>(`/capex-versions/${v.id}/allocations`);
        ...
        if (loadedMethod === 'manual_company') {
          try {
            ...
            setRows(rowsFromApi);
            setError(err?.message || t('capex.allocations.failedToCompute'));
          }
      ...
      setError(getApiErrorMessage(e, t, t('capex.allocations.failedToLoad')));
    } finally {
      setLoading(false);
    }
  ...
  const fullPrefill = async (driver: 'headcount' | 'it_users' | 'turnover') => {
    try {
      ...
      if (sum <= 0) { setError(t('capex.allocations.provideDriverSumZero', { driver })); return; }
      let accPct = 0;
      ...
      setError(getApiErrorMessage(e, t, t('capex.allocations.failedToCompute')));
    }
  };

  // Auto-prefill manual company rows with all enabled companies once per session.
  ...
  const handleCompanyChange = (idx: number, companyId: string | null) => {
    ...
    console.debug('[CapexAllocationEditor] company change', idx, companyId, next);
    setRows(next);
    setHasUnsavedChanges(true);
    if (isManualCompany) {
  ...
  const handleDepartmentChange = (idx: number, deptId: string | null) => {
    ...
    console.debug('[CapexAllocationEditor] department change', idx, deptId, next);
    setRows(next);
    setHasUnsavedChanges(true);
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!v.allocation_method || v.allocation_method !== method || (v as any).allocation_driver !== driverForSave) {
        await api.patch(`/capex-items/${id}/versions`, { id: v.id, allocation_method: method, allocation_driver: driverForSave });
      }
      ...
      if (!isAuto) {
        console.debug('[CapexAllocationEditor] rows for save', currentRows, 'payload', payload);
      }
      if (!isAuto) {
        if (payload.length === 0) {
          throw new Error(isManualDept ? t('capex.allocations.selectAtLeastOneDept') : t('capex.allocations.selectAtLeastOneCompany'));
        }
      }
      console.debug('[CapexAllocationEditor] Saving allocations', { method, isAuto, versionId: v.id, payload, driver: driverForSave });
      await api.post(`/capex-versions/${v.id}/allocations/bulk-upsert`, payload);
      if (!isAuto) {
        const verification = await verifyManualSave(v.id, payload);
        if (!verification.ok) {
          const reason = verification.error?.message
            ? verification.error.message
            : t('capex.allocations.verificationMismatch');
          throw new Error(t('capex.allocations.verificationFailed', { reason }));
        }
      }
      setHasUnsavedChanges(false);
      await load();
    } catch (e: any) {
      setError(getApiErrorMessage(e, t, t('capex.allocations.failedToSave')));
      throw e;
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Stack spacing={2}>
      ...
      <Typography variant="subtitle2">{t('capex.allocations.title')}</Typography>
