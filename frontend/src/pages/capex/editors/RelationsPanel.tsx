        ...
        <Box component="form" onSubmit={saveLinkDraft}>
          ...
          <DialogContent>
            <Stack spacing={2} sx={{ pt: 1 }}>
              <TextField autoFocus aria-label={t('capex.relations.linkName', 'Name')} placeholder={t('capex.relations.linkDescriptionPlaceholder', 'e.g., vendor portal')} value={linkDraft.description} onChange={(e) => setLinkDraft((p) => ({ ...p, description: e.target.value }))} variant="standard" />
              <TextField aria-label={t('capex.relations.linkUrl', 'URL')} placeholder="https://..." value={linkDraft.url} onChange={(e) => setLinkDraft((p) => ({ ...p, url: e.target.value }))} variant="standard" />
            </Stack>
          </DialogContent>
          <DialogActions>
