...
| `/ops/tasks` | `frontend/src/pages/TasksPage.tsx` | Task management grid with ServerDataGrid. Shows all tasks across OPEX and Contracts with advanced filtering, quick search, status tracking (open, in_progress, done, cancelled), and responsible person assignment. Default filter shows open tasks sorted by creation date descending. | Requires `reader` on `tasks`; auto-granted when any operations resource (opex, contracts) has access |
...
| `/it/applications` | `frontend/src/pages/it/ApplicationsPage.tsx` | IT Operations: **Apps & Services** grid. Default lifecycle filter hides Retired (but Retired remains selectable). Closed-set columns use checkbox-set filters (Category, Environments, Lifecycle, Criticality, Hosting, External Facing, SSO, MFA, Data Class, Contains PII) with scoped values; Environments reflect **active** instances (matching the chips), Hosting is derived from server locations, and Data Class can be blank. Columns include environment chips, suite/component counts, owners, compliance fields, counts, vendor data, and the catalog type badge. Actions: New App/Service, Import CSV, Export CSV, Copy item, Delete Selected. Column chooser toggles include flags (instances/structure/counts/etc.), letting the UI request the necessary expansions lazily. | `applications` (`reader` for list; `manager` to create/update/delete) |
...
| `/it/settings` | `frontend/src/pages/it/ItOperationsSettingsPage.tsx` | IT Operations Settings page for configuring tenant-wide lists. Now includes collapsible sections and a new **Operating Systems** catalog (name + standard/extended support dates) that feeds the Asset workspace OS selector. Existing lists: Data Classes, Asset Types/Roles, Hosting Types (on-prem/cloud category), Cloud Providers, Lifecycle, Interface enums. | `settings` (`reader` to view, `admin` to edit) |
...
- Grid: `frontend/src/pages/portfolio/RequestsPage.tsx` with status, type, category filters
...
- Grid: `frontend/src/pages/portfolio/ProjectsPage.tsx` with progress bar column
