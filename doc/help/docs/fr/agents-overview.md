...
Les administrateurs disposent d'un bouton **Nouvel agent** en haut à droite de la page. Il ouvre un assistant court :

1. **Type** — donnez à l'agent un **Nom** et une **Description**, et choisissez le **Type d'agent** : **Helpdesk** ou **Supervision d'infrastructure (SRE)**. Le nom et la description sont pré-remplis avec des valeurs raisonnables, remplacées si vous changez de type sans les avoir modifiées vous-même.
2. **Connexion** — le système sur lequel l'agent travaille : le système de tickets (**GLPI**) pour un agent helpdesk, ou l'**Outil de supervision** pour un agent SRE. Un lien **Gérer les intégrations** mène à **Administration → Intégrations** si la connexion n'est pas encore configurée. Si aucun outil de supervision n'est connecté, l'agent est tout de même créé — il reste simplement inactif jusqu'à ce qu'il y en ait un.
3. **Surveillance** (helpdesk uniquement) — **Surveiller les nouveaux tickets** (désactivé par défaut) détermine si l'agent devra à terme surveiller de lui-même, et le reste de l'étape décide des tickets qu'il cible. Des préréglages (**Nouveaux tickets**, **Tous ouverts**, **Traités par cet agent**) vous donnent un point de départ ; le générateur de filtres l'affine davantage, tous les filtres étant combinés et leurs valeurs issues du système de tickets connecté. Dans tous les cas, l'agent est créé en **Non démarré** : rien ne s'exécute tant que vous n'avez pas défini son mode d'exécution. Un agent de supervision saute cette étape : vous choisirez les alertes qu'il surveille dans ses paramètres, ensuite.
4. **Limites** — le cadre de sécurité : **Priorité agent**, **Réviser toutes les (heures)**, la gestion de la **Collision ticket**, **Tickets max par contrôle** et **Requêtes fournisseur max**, la **Fenêtre d'approbation (heures)**, le comportement **Si le ticket a changé**, ainsi que les plafonds par exécution et quotidiens sur les **Tokens**, le **Coût** et les **Exécutions**. Le modèle est livré avec des valeurs par défaut raisonnables ; chaque champ est expliqué dans l'onglet **Paramètres** de l'[Espace de l'agent](agents-workspace.md).
5. **Vérification** — un récapitulatif de tout ce qui précède.

Les nouveaux agents sont toujours créés en **Non démarré**, et vous arrivez sur leur onglet **Paramètres**. La démarche recommandée est de terminer la configuration, de placer l'agent en **Manuel uniquement** et de le tester sur de vrais tickets, puis de le passer en **Surveillance** une fois que vous avez confiance en sa production.
...
- **Manuel uniquement avant Surveillance.** Un agent créé par l'assistant est intentionnellement en **Non démarré**. Exécutez-le d'abord manuellement sur une poignée de tickets représentatifs depuis son espace ; ne le passez en **Surveillance** qu'une fois satisfait de ce qu'il rédige.
