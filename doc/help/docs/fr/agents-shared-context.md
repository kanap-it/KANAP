...
Les profils résident ici, mais ils s'activent agent par agent depuis l'onglet **Paramètres** de l'agent concerné (voir [Espace de l'agent](agents-workspace.md)). Dans la section **Contexte partagé** de l'agent, vous activez **Utiliser le contexte partagé** et choisissez un profil. Tant que vous ne l'avez pas fait, l'agent affiche **Aucun profil sélectionné** et fonctionne sans aucun contexte partagé.
