...
Navigate to **Administration** from the main menu to access the administration hub.
...
|--------|-------------|------------|
| **Nouveau** | Create a new user | `users:member` |
| **Edit** | Modify user details (click any row) | `users:member` |
| **Importer CSV** | Bulk import users | `users:admin` |
| **Exporter CSV** | Export user list | `users:admin` |
| **Invite** | Send login invitations to selected users | `users:admin` |
| **Supprimer** | Remove selected users permanently | `users:admin` |
...
4. Click **Enregistrer** or **Save and Invite** to send login email
...
- **Administration**: Full access including delete
...
- **Supprimer**: Remove a custom role (only if no users are assigned)
...
Click **Save changes** to update both customer and invoicing details. Use **Réinitialiser** to discard unsaved edits.
...
Use **Administration > Personnalisation** to apply your company identity in KANAP.
