...
Next to the instructions, **Names you can use in the instructions** lists ticket statuses and their keys, technician groups and the technicians themselves when **Assignment** is enabled, and categories, priorities and types when **Classification** is enabled. Use the displayed names or enum keys. Search categories or technicians to narrow the list. The search covers all categories; when there are more than 200, the agent first narrows its choices to the category subtrees in your targeting, then keeps the first 200 full names alphabetically. The run trace reports any remaining truncation.
