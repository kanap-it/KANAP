...
Los perfiles residen aquí, pero se activan por agente desde la pestaña **Configuración** de ese agente (consulte [Espacio del agente](agents-workspace.md)). En la sección **Contexto compartido** del agente, active **Usar contexto compartido** y elija un perfil. Hasta que lo haga, el agente muestra **Ningún perfil seleccionado** y funciona sin ningún contexto compartido.
