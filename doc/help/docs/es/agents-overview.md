...
Los administradores disponen de un botón **Nuevo agente** en la parte superior derecha de la página. Abre un breve asistente:

1. **Tipo** — dé al agente un **Nombre** y una **Descripción**, y elija el **Tipo de agente**: **Helpdesk** o **Supervisión de infraestructura (SRE)**. El nombre y la descripción vienen rellenados con valores razonables, que se sustituyen si cambia de tipo y no los ha editado usted mismo.
2. **Conexión** — el sistema con el que trabaja el agente: el sistema de tickets (**GLPI**) para un agente helpdesk, o la **Herramienta de supervisión** para uno SRE. Un enlace **Gestionar integraciones** lleva a **Administración → Integraciones** si la conexión aún no está configurada. Si no hay ninguna herramienta de supervisión conectada, el agente se crea igualmente: simplemente permanece inactivo hasta que la haya.
3. **Vigilancia** (solo helpdesk) — **Vigilar nuevos tickets** (desactivado por defecto) determina si el agente acabará vigilando por su cuenta, y el resto del paso decide a qué tickets apunta. Los preajustes (**Tickets nuevos**, **Todos abiertos**, **Gestionados por este agente**) le dan un punto de partida; el generador de filtros lo acota aún más, con todos los filtros combinados y sus valores tomados del sistema de tickets conectado. En cualquier caso el agente se crea como **No iniciado**, así que no se ejecuta nada hasta que fije su modo de ejecución. Un agente de supervisión se salta este paso: las alertas que vigila se eligen después en su configuración.
4. **Límites** — el marco de seguridad: **Prioridad del agente**, **Revisar cada (horas)**, la gestión de **Colisión de ticket**, **Tickets máximos por comprobación** y **Solicitudes máximas al proveedor**, la **Ventana de aprobación (horas)**, el comportamiento **Si el ticket cambió** y los topes por ejecución y diarios de **Tokens**, **Coste** y **Ejecuciones**. La plantilla incluye valores predeterminados razonables; cada campo se explica en la pestaña **Configuración** del [Espacio del agente](agents-workspace.md).
5. **Revisión** — un resumen de todo lo anterior.

Los agentes nuevos siempre se crean como **No iniciado**, y usted aterriza en su pestaña **Configuración**. El camino recomendado es terminar la configuración, poner el agente en **Solo manual** y probarlo con tickets reales, y pasarlo a **Vigilancia** una vez que confíe en su salida.
...
- **Solo manual antes que Vigilancia.** Un agente creado por el asistente está intencionadamente en **No iniciado**. Ejecútelo primero manualmente sobre un puñado de tickets representativos desde su espacio; páselo a **Vigilancia** solo cuando esté satisfecho con lo que redacta.
