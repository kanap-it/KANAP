...
Profile befinden sich hier, werden aber pro Agent über die Registerkarte **Einstellungen** dieses Agenten aktiviert (siehe [Agent-Arbeitsbereich](agents-workspace.md)). Im Abschnitt **Gemeinsamer Kontext** des Agenten aktivieren Sie **Gemeinsamen Kontext verwenden** und wählen ein Profil. Bis dahin zeigt der Agent **Kein Profil ausgewählt** an und läuft ohne jeglichen gemeinsamen Kontext.
