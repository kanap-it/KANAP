...
Administratoren erhalten oben rechts auf der Seite eine Schaltfläche **Neuer Agent**. Sie öffnet einen kurzen Assistenten:

1. **Typ** – geben Sie dem Agenten einen **Namen** und eine **Beschreibung** und wählen Sie den **Agententyp**: **Helpdesk** oder **Infrastruktur-Überwachung (SRE)**. Name und Beschreibung sind mit sinnvollen Standardwerten vorbelegt, die ausgetauscht werden, wenn Sie den Typ wechseln und sie nicht selbst bearbeitet haben.
2. **Verbindung** – das System, mit dem der Agent arbeitet: das Ticketsystem (**GLPI**) bei einem Helpdesk-Agenten oder das **Überwachungstool** bei einem SRE-Agenten. Ein Link **Integrationen verwalten** springt zu **Administration → Integrationen**, falls die Verbindung noch nicht eingerichtet ist. Ist kein Überwachungstool verbunden, wird der Agent dennoch erstellt – er bleibt lediglich inaktiv, bis eines verbunden ist.
3. **Überwachung** (nur Helpdesk) – **Neue Tickets überwachen** (standardmäßig aus) legt fest, ob der Agent später von sich aus beobachten soll; der Rest des Schritts bestimmt, welche Tickets er anvisiert. Voreinstellungen (**Neue Tickets**, **Alle offenen**, **Von diesem Agenten bearbeitet**) geben Ihnen einen Ausgangspunkt; der Filtereditor grenzt sie weiter ein, wobei alle Filter kombiniert werden und ihre Werte aus dem verbundenen Ticketsystem stammen. In beiden Fällen wird der Agent als **Nicht gestartet** angelegt – es läuft nichts, bevor Sie seinen Ausführungsmodus gesetzt haben. Ein Überwachungs-Agent überspringt diesen Schritt: Welche Alarme er beobachtet, legen Sie danach in seinen Einstellungen fest.
4. **Limits** – der Sicherheitsrahmen: **Agentenpriorität**, **Prüfen alle (Stunden)**, die Behandlung von **Ticket-Kollision**, **Max. Tickets pro Prüfung** und **Max. Provider-Anfragen**, das **Genehmigungsfenster (Stunden)**, das Verhalten **Wenn Ticket geändert wurde** sowie die Limits pro Lauf und pro Tag für **Tokens**, **Kosten** und **Läufe**. Die Vorlage liefert sinnvolle Standardwerte; jedes Feld ist auf der Registerkarte **Einstellungen** im [Agent-Arbeitsbereich](agents-workspace.md) erklärt.
5. **Prüfen** – eine Zusammenfassung all dessen, was oben steht.

Neue Agenten werden immer als **Nicht gestartet** erstellt, und Sie landen auf ihrer Registerkarte **Einstellungen**. Der empfohlene Weg ist, die Einrichtung abzuschließen, den Agenten auf **Nur manuell** zu stellen und ihn an echten Tickets zu testen und ihn erst dann auf **Beobachtung** zu setzen, wenn Sie seiner Ausgabe vertrauen.
...
- **Nur manuell vor Beobachtung.** Ein vom Assistenten erstellter Agent ist absichtlich **Nicht gestartet**. Lassen Sie ihn zuerst von Hand an einigen repräsentativen Tickets aus seinem Arbeitsbereich laufen; setzen Sie ihn erst dann auf **Beobachtung**, wenn Sie mit dem, was er entwirft, zufrieden sind.
