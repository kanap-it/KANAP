...
- **Neue Assets**: Objekte ohne Gegenstück in KANAP. Sie werden angelegt.
- **Aktualisierte Assets**: Objekte, die einem vorhandenen Asset zugeordnet sind und davon abweichen. Jedes listet die Felder auf, die sich ändern, Feld für Feld, als `vorher → nachher`.
- **Zu entscheiden**: Objekte, über die KANAP nicht selbst entscheidet. Mit ihnen geschieht nichts, bis Sie sie auf der Seite klären.
...
Lesen Sie die Vorschau und drücken Sie dann **Anwenden**. Objekte, die bereits identisch sind, werden als unverändert gezählt und gar nicht angefasst. Bei großen Inventaren listet die Vorschau nur die ersten 500 Zeilen auf. Der Lauf selbst wendet alles an.
...
Aktivieren Sie **Automatische Synchronisierung** auf der Integrationskarte, und KANAP führt denselben Lauf stündlich aus und wendet die Änderungen ohne Vorschau an. Der Schalter gilt pro Mandant, und nur Mandanten, die ihn aktiviert haben, werden angefahren.
...
|-------|---------------|-----------------|
| **Zu entscheiden** | Mehrere Assets kommen für dieses Objekt infrage, oder zwei Objekte haben dasselbe Asset erreicht. | **Verknüpfen mit...** einem der Kandidaten, **Neues Asset anlegen** oder **Ignorieren**. |
