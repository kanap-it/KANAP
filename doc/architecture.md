...
Every table with a `tenant_id` column has `ENABLE` + `FORCE ROW LEVEL SECURITY` and a `USING`/`WITH CHECK` policy binding `tenant_id` to the session tenant (212 tables on 2026-09-19, none enabled without force). Since migration `1853580000000` no policy casts the raw setting any more: 192 read the tenant through `app_current_tenant()` and 20 keep the equivalent `NULLIF(current_setting('app.current_tenant', true), '')::uuid` form. Both yield NULL when the context is missing or empty, so every row is filtered out instead of raising `invalid input syntax for type uuid`, and neither falls back to another tenant. Families covered today:
