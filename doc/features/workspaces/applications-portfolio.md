...
- `applications`: `{ id, tenant_id, name, supplier_id?, description?, editor?, retired_date?, version?, end_of_support_date?, go_live_date?, predecessor_id?, lifecycle, criticality?, business_mtd_minutes?, legacy_criticality?, business_criticality_origin, cyber_criticality?, recovery_wave?, rto_minutes?, rpo_minutes?, classification_justification?, classification_revision, classification_review?, data_class?, external_facing, is_suite, last_dr_test?, etl_enabled, access_methods(text[]), contains_pii, licensing?, notes?, support_notes?, users_mode('manual'|'it_users'|'headcount'), users_year, users_override?, status, disabled_at, created_at, updated_at }`.

  - V1 classification rules: `criticality` is a nullable, read-only projection of `business_mtd_minutes` using the tenant catalog; `legacy_criticality` preserves the pre-migration value. The UI, API, CSV, and Plaid require a configured `businessMtdPresets` value for a new or changed MTD. Existing non-preset values remain valid when unchanged, including catalog recalculation. Cyber criticality, data confidentiality, and recovery wave are independent choices. RTO/RPO remain free integer minutes (RPO may be zero), and absent values remain `null`.
  - `classification_review` stores the server actor/date, application revision, and catalog versions. The derived review state is incomplete until MTD, cyber criticality, data class, recovery wave, and justification are present; semantic changes make a review stale. Copies reset review and last recovery test.
