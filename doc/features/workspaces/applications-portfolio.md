...
- `applications`: `{ id, tenant_id, name, supplier_id?, description?, editor?, retired_date?, version?, end_of_support_date?, go_live_date?, predecessor_id?, lifecycle, criticality?, cyber_criticality?, recovery_wave?, rto_minutes?, rpo_minutes?, classification_justification?, classification_revision, classification_review?, data_class?, external_facing, is_suite, last_dr_test?, etl_enabled, access_methods(text[]), contains_pii, licensing?, notes?, support_notes?, users_mode('manual'|'it_users'|'headcount'), users_year, users_override?, status, disabled_at, created_at, updated_at }`.

  - Classification rules: `criticality`, `cyber_criticality`, `data_class` and `recovery_wave` are nullable codes chosen from the tenant catalog (`classificationPatch` accepts a code or an unambiguous label, and refuses a deprecated level unless the record already uses it). Catalog edits never rewrite them. RTO/RPO remain free integer minutes (RPO may be zero), and absent values remain `null`.
  - `classification_review` stores the server actor/date and the application revision. The derived review state is incomplete until business criticality, cyber criticality, data class, recovery wave, and justification are present; a later classification change (revision bump, including recovery references and data residency through a trigger) makes a review stale. Copies reset review and last recovery test.
